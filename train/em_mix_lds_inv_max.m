function [lambda, loglik_out]=em_mix_lds_inv_max(data, n_comp, varargin)
% EM_MIX_LDS fits a Gaussian mixture of stable linear 
% dynamical systems to data with the EM algorithm.
%   USAGE:
%   [lambda, loglik_out] = EM_MIX_LDS_INV_MAX(data, n_comp) fits a mixture 
%   of linear dynamical systems to the data and returns a local optimum of the
%   maximum likelihood optimization problem. The attractor is estimated 
%   considering an inverse LDS mixture to approx. the problem to a convex
%   formulation. 
%
%   INPUT PARAMETERS:
...
lambda= init_kmeans_mix_lds_inv_max(data, n_comp, options);
...
for it = 1:options.n_iter
    ...
    [lambda.x_attractor, lambda.A] = estimate_stable_mix_lds_inv_max( ...
