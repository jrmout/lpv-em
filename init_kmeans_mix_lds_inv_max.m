function [ lambda ] = init_kmeans_mix_lds_inv_max(data, n_comp, options)
%INIT_KMEANS_MIX_LDS Initializes the model with kmeans and solves the
%resulting clusters with estimate_stable_mix_lds_inv_max. 

d=size(data,1)/2;
x_obs = data(1:d,:)';
x_dot_obs = data(d+1:end,:)';
dir_x_dot = x_dot_obs ./ ...
    repmat(max(sqrt(sum(x_dot_obs.^2,2)), realmin),1,size(x_dot_obs,2));
